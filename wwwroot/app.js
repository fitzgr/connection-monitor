...
function uptimeChart(items){const c=$('uptimeChart'),[g,w,h,d]=size(c),hours=+$('range').value,cut=Date.now()-hours*3600000,data=items.filter(x=>new Date(x.timestamp)>=cut),barY=6,barHeight=Math.max(14,h-26);g.scale(d,d);g.fillStyle=css('--unobserved');g.fillRect(0,barY,w,barHeight);if(!data.length)return;const bar=Math.max(1,w/data.length);data.forEach((s,i)=>{connectionBar(g,i*bar,barY,Math.ceil(bar),barHeight,s.online)});g.fillStyle=css('--muted');g.font='10px system-ui';g.fillText(`${hours} hours ago`,0,h-2);g.fillText('Now',w-22,h-2)}
